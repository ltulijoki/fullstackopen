...
const Persons = ({ persons, search, setPersons, setMessage, setType }) => {
  ...
  return (
    <div>
      {filtered.map(person =>
        <ShowNumber person={person} persons={persons} setPersons={setPersons} id={person.id} key={person.id} setMessage={setMessage} setType={setType} />
      )}
    </div>
  )
}
...
const add = (event, persons, newName, newNumber, setPersons, setNewName, setNewNumber, setMessage, setType) => {
  ...
  if (!end) {
    ...
    PersonService.add(obj).then(res => {
